...
coefs <- tibble(coef = names(mod$coefficients),
                estimate = unname(mod$coefficients),
                se = unname(summary(mod)$tTable[,"Std.Error"]),
...
coxmodel_train <- coxph(y[-index] ~ kdm3a.kdm3a_mut  + lymph_nodes_examined_positive +
                      treatment      +     as.factor(integrative_cluster5) +
                    as.factor(integrative_cluster3) +
                          gsk3b      +      treatment.tumor_size +
                          prkg1     +             treatment.e2f8 +
              ...
              treatment.ugt2b17             +               igf1 +
                ...
                gata3.gata3_mut             +             acvr1c +
                 ...
                 treatment.cul1 + as.factor(pam50_._claudin.low_subtypeLumA) +
                birc6.birc6_mut           +    treatment.cyp11a1 +
                  tbx3.tbx3_mut           +       treatment.mapt, weights = W.vec[-index], data = data_matrix[-index,])



pred_test_cox <- predict(coxmodel_train, newdata=data_matrix[index,], type='risk', se=FALSE)

get.concordance(pred_test = -pred_test_cox, truth_test = truth_test, death = death)
