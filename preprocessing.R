...
beta.nonzero = cox.fit$beta[,1][abs(cox.fit$beta[,1]) > 1e-10]

cox.fit.boot$beta[,1][abs(cox.fit$beta[,1]) > 1e-10]
...
for (i in 1:1000)
{
  ...
  beta_mat[i,] = model$beta[,1]
}

CI_mat = array(rep(NA, p*2), c(p,2))
for (j in 1:p)
{
  CI_mat[j,] = quantile(beta_mat[,j], c(0.025, 0.975))
}

nonzero.ind = which(abs(cox.fit$beta[,1]) > 1e-10)
CI_mat[nonzero.ind,]
beta.nonzero
